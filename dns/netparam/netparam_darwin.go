...
func isGatewaysFieldDelimiter(r rune) bool {
	// this is guessed
...
func GetDhcpNameservers() (nameservers []string, domain string, searchs []string) {
	...
	if err != nil {
		log.Warnln("GetDhcpNameservers() route: %s", err.Error())
	...
	if err != nil {
		log.Warnln("GetDhcpNameservers() ipconfig getpacket: %s", err.Error())
		return []string{}, "", []string{}
	}
...
func GetGateways() (gateways []string) {
	out, err := exec.Command("route", "-n", "get", "default").Output()
	if err != nil {
		log.Warnln("GetGateways() route: %s", err.Error())
		return []string{}
	}
	...
	return strings.FieldsFunc(gwss, isGatewaysFieldDelimiter)
}
